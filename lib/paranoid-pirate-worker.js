...
var DEFAULT_BACKOFF_OPTIONS = { backoff: 100, retries: 10 };
...
function _createMessageHandler(instance) {
    return function() {
        ...
        if(action === _messageDefinitions.HEARTBEAT) {
            ...
            this.emit(_eventDefinitions.REQUEST, frames.splice(1));
        }
    }.bind(instance);
}
...
function ParanoidPirateWorker(endpoint, monitor, backoffOptions, socketOptions) {
    ...
    this._hbResponseReceived = false;
}
...
ParanoidPirateWorker.prototype._initialize = function(emitEvent) {
    ...
    this._socketManager.socket.send([EMPTY_BUFFER, _messageDefinitions.READY]);
    if(emitEvent) {
...
ParanoidPirateWorker.prototype._send = function(message) {
    if(message instanceof Array) {
        message.unshift(EMPTY_BUFFER);
    } else {
        message = [ EMPTY_BUFFER, message ];
    }
