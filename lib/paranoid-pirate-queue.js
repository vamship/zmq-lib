...
function _swapFrames(frames, firstIndex, secondIndex) {
    var temp = frames[firstIndex];
...
ParanoidPirateQueue.prototype._getRequestForWorker = function(worker) {
    ...
    for (var index = 0; index < this._pendingRequests.length; index++) {
        ...
        if (!this._session.isEnabled) {
            var requestService = pendingRequest.data[1].toString();
            if (worker.services.length === 0 || worker.services.indexOf(requestService) >= 0) {
                targetIndex = index;
                break;
            }
        } else if (pendingRequest.session.workerId === '') {
            targetIndex = index;
            break;
...
ParanoidPirateQueue.prototype._getWorkerForRequest = function(request) {
    var worker = null;
    var requestService = request.data[1].toString();
    ...
    if (request.session && request.session.workerId !== '') {
        ...
        for (var index = 0; index < this._availableWorkers.length; index++) {
            worker = this._availableWorkers[index];

            //If we can't find an entry in the worker map, it means that the
            //worker has expired and has been removed.
            worker = this._workerMap[worker.id];
            if (!worker) {
                this._availableWorkers.splice(index, 1);
            } else {
                if (worker.services.length === 0 || worker.services.indexOf(requestService) >= 0) {
                    this._availableWorkers.splice(index, 1);
                    break;
 ...
 * @protected
 * @param {Object} request A request object that represents the client request.
 * @return {Object} An object that represents the client session. If session
 *                  is not enabled, a null will be returned.
 */
ParanoidPirateQueue.prototype._getClientSession = function(request) {
...
ParanoidPirateQueue.prototype._getFrontEndMessageHandler = function() {
    return function() {
        ...
        if (frames.length < 4 || frames[2].toString() !== _messageDefinitions.REQUEST) {
            return;
        } else {
            // Swap frames [2, 3] with frames [0, 1]. Frames [0, 1] are inserted by the queue,
            // and contain addressing information, [2, 3] contains message header information,
            // and needs to be moved to the top of the message sent to the worker.
            _swapFrames(frames, 0, 2);
            _swapFrames(frames, 1, 3);
        }
        var request = {
            clientAddress: frames[2],
            timestamp: Date.now(),
            data: frames
        };

        this.emit(_eventDefinitions.REQUEST, request.clientAddress, request.data);

        request.session = this._getClientSession(request);
        if (request.session) {
...
ParanoidPirateQueue.prototype._getBackEndMessageHandler = function() {
    return function() {
        ...
        if (!worker && action !== _messageDefinitions.READY) {
            // Unkown worker sending an unreognized message. Discard.
            return;
        }
        ...
        switch (action) {
            ...
            case _messageDefinitions.READY:
                ...
                for (var index = 1; index < frames.length; index++) {
                    var service = frames[index].toString();
                    if (service !== '') {
                        worker.services.push(frames[index].toString());
                    }
                }
                this._workerMap[workerId] = worker;
                break;
            case _messageDefinitions.RESPONSE:
                worker.lastAccess = Date.now();
                worker.isAvailable = true;
                var response = frames.shift();
                frames.splice(2, 0, response);
                this._frontEnd.socket.send(frames);
                this.emit(_eventDefinitions.ASSIGNED_RESPONSE, frames[0], frames);
                break;
            default:
                // Unrecognized message. Discard.
                return;
        }
