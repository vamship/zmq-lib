...
function ParanoidPirateQueue(feEndpoint, beEndpoint, workerOptions, feOptions, beOptions) {
    ...
    var session = workerOptions.session || {};
    this._session = {
    ...
    return function() {
        ...
        if (worker) {
            if(worker.isAvailable) {
                ...
                this._sendToWorker(worker, clientSession, frames);
            } else {
                this._pendingRequestCount++;
                worker.pendingRequests.push(frames);
            }
        } else {
            this._pendingRequestCount++;
...
ParanoidPirateQueue.prototype._getBackEndMessageHandler = function() {
    return function() {
        ...
        if (request) {
            var clientSession = this._getClientSession(request);
            this._pendingRequestCount--;

            this._sendToWorker(worker, clientSession, request);
        } else {
            this._availableWorkerCount++;
