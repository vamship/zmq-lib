...
ParanoidPirateQueue.prototype._getRequestForWorker = function(worker) {
    ...
    for(var index=0; index<this._pendingRequests.length; index++) {
        ...
        if(!this._session.isEnabled
            || pendingRequest.session.workerId === '') {
            requestIndex = index;
            break;
    ...
    var request = this._pendingRequests[requestIndex];
    if(request) {
...
ParanoidPirateQueue.prototype._getWorkerForRequest = function(request) {
    ...
    //Need to check worker id as well, to account for situations
    //when the worker is dead, but the client session is still attached
    //to it. In those cases worker will be undefined, but
    //request.session.workerId will be valid.
    if(!worker && (!request.session || request.session.workerId === '')) {
...
ParanoidPirateQueue.prototype._getFrontEndMessageHandler = function() {
    return function() {
        ...
        request.worker = this._getWorkerForRequest(request);

        if (request.worker) {
            if(request.worker.isAvailable) {
                this._availableWorkerCount--;
                this._sendToWorker(request.worker, request);
            } else {
                this._pendingRequests.push(request);
                request.worker.pendingRequestCount++;
            }
        } else {
            this._pendingRequests.push(request);
        }
